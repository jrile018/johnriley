...
export default function SectionWrapper({ id, title, children, className = "", compact = false }: SectionWrapperProps) {
  return (
    <section
      className={`h-full px-4 sm:px-6 lg:px-8 overflow-hidden ${className}`}
    >
      <div className={`max-w-5xl mx-auto h-full flex flex-col ${compact ? "py-1" : "py-6"}`}>
        <div className="bg-surface/50 border border-border rounded-md overflow-hidden scanlines backdrop-blur-xs flex-1 min-h-0 flex flex-col">
          ...
          {/* Content */}
          <div className={`${compact ? "p-2 sm:p-2.5" : "p-4 sm:p-6"} relative z-[2] flex-1 min-h-0 overflow-hidden flex flex-col`}>
